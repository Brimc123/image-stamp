"""
RETROFIT DESIGN TOOL - FIXED FUNCTION NAMES FOR MAIN.PY
✅ Function names match main.py imports exactly
✅ Generic installation instruction filenames
"""
...
# ============================================================================
# MAIN ROUTES (FUNCTION NAMES MATCH MAIN.PY EXACTLY)
# ============================================================================

def get_retrofit_tool_page(request: Request):
    """Phase 1: Initial page - upload documents & select measures"""
    ...
    # Generate measure selection cards
    measure_cards = ""
...
async def post_retrofit_process(request: Request):
    """Process uploads and extract data, then redirect to questions"""
    user_row = require_active_user_row(request)
    if isinstance(user_row, (RedirectResponse, HTMLResponse)):
        return user_row
...
def get_calc_upload_page(request: Request):
    """Page for uploading calculations (if required)"""
    user_row = require_active_user_row(request)
    if isinstance(user_row, (RedirectResponse, HTMLResponse)):
        return user_row
    ...
    upload_boxes = ""
    for m in measures_needing_calcs:
        measure = MEASURES[m]
        upload_boxes += f"""
        <div class="upload-box" id="calc{m}Box" onclick="document.getElementById('calc{m}').click()">
            <div class="upload-icon">{measure['icon']}</div>
            <div class="upload-text">{measure['name']} Calculations</div>
            <div class="upload-subtext">Upload calculation file</div>
            <div class="file-name" id="calc{m}Name"></div>
        </div>
        <input type="file" id="calc{m}" name="calc_{m}" accept=".pdf,.xlsx,.xls">
        """
...
async def post_calc_upload(request: Request):
    ...
    # Store calc uploads (for now just acknowledge they were uploaded)
    # In future, extract data from these files
...
<body>
    <div class="container">
        ...
        <p style="margin-bottom: 20px; color: #666;">Measure {current_index + 1} of {len(selected_measures)}</p>
